...
class FIMTDD:
    ...
    def __init__(self,gamma=0.01,n_min = 50,alpha=0.005,threshold=50):
        """
        ...
        :return:
        """
        self.root = LeafNode(self,n_min=n_min,gamma=gamma,alpha=alpha,threshold=threshold)
        self.gamma = gamma
        self.alpha = alpha
...
class Node:
    ...
    def __init__(self,parent,key=None,key_dim=0,left=None,right=None,alpha=0.005,threshold=50,n_min=100,gamma=0.01):
        """
    ...
    def grow_alt_tree(self):
        ...
        self.alt_tree = LeafNode(self,self.n_min,None,self.gamma,self.alpha,threshold=self.threshold)
    ...
    def detect_change(self,y,yp):
        ...
        if self.PH < self.minPH or self.minPH == None:
            self.minPH = self.PH
...
class LeafNode(Node):
    ...
    """
    def __init__(self,parent,n_min,model=None,gamma=0.01,alpha=0.005,threshold=50):
        """
    ...
    def split(self,splits,index):
        ...
        """
        #return
        print "splitting node: "+str(self.index)
        node = Node(self.parent,key_dim=index,key=splits['bestsplit'],gamma=self.gamma)
        left = LeafNode(parent=node,n_min=self.n_min,gamma=self.gamma,alpha=self.alpha)
        right = LeafNode(parent=node,n_min=self.n_min,gamma=self.gamma,alpha=self.alpha)
        l1 = LinearRegressor(left,self.model.w)
        l2 = LinearRegressor(right,self.model.w)
        left.model = l1
        right.model = l2
        #left.index += 1
        #right.index += 2
    ...
    def findBest(self,splits):
        ...
        for i in range(len(splits)):
            m = splits[i]['max']
            if max_index == None or m > max_index:
    ...
    def findBestSplit(self,tree,sdr = None):
        ...
        if(sdr['max'] == None or new_sdr > sdr['max']):
            sdr['2nd'] = sdr['max']
            sdr['max'] = new_sdr
            try:
                if not new_sdr == 0.0:
...
class LinearRegressor:
    ...
    def __init__(self,leafnode,w=None):
    ...
    def learn(self,x,y,yp):
        learn = (self.i_learn)/(1+(self.c*self.l_decay))
        delta = learn * (y - yp)*x
        self.w += delta
        pass
...
class Node_EBST:
    ...
    def add(self,val,y):
        if val <= self.key:
            ...
            self.l_y += y
            self.l_y_sq += y**2
            if self.left == None and val != self.key:
